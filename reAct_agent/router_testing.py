...
class Route(BaseModel):
    llm_choice: Literal["Time", "Place", "Airline"] = Field(
         description= "Next step in routing workflow"
    )
...
# Lets get all tools in one list which we can refer to in a modular way
...
async def llm_call_router(state: State):
    ...
    decision = await router.ainvoke([
        ...
        SystemMessage(
            content = "You are a router that classifies user queries into one of three categories: 'Time', 'Place', or 'Airline'. "
        "Return a JSON object with a single key 'llm_choice' and a value that is exactly 'Time', 'Place', or 'Airline'. "
        "For time-related queries (e.g., 'best time to travel', 'weather timing'), return {'llm_choice': 'Time'}. "
        "For destination or place-related queries (e.g., 'best places to visit', 'shopping in Japan'), return {'llm_choice': 'Place'}. "
        "For airline or flight-related queries (e.g., 'best airlines', 'flight costs'), return {'llm_choice': 'Airline'}. "
        "If unsure, choose the most relevant category based on the query's intent. "
        "Do not return any other values or formats."
        ),
...
graph_builder.add_node(node= "tools", action = ToolNode(get_tools()))
...
→ Classifies query as related to llm_Time

Flow goes to llm_Time
→ It analyzes and triggers the tools_condition, which returns tool_Time is needed

Flow goes to tool_Time
→ Fetches actual time in Tokyo

Result goes back to llm_Time
→ LLM formats: “It’s 9:00 PM in Tokyo right now.”

Ends.
"""

graph_builder.add_edge(start_key = START, end_key= "llm_call_router")
...
graph_builder.add_edge("tools", "llm_Time")
graph_builder.add_edge("tools","llm_Place")
graph_builder.add_edge("tools","llm_Airline")
...
async def main():
    state = await router_workflow.ainvoke({"user_input": "Best time to travel to Japan?"})
    print("Full state returned:", state)
    print("llm_final_answer:", state.get("llm_final_answer", "No final answer found."))
